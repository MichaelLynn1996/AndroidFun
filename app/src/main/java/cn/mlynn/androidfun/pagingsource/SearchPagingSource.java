 ...
 * @ClassName: SearchPagingSource
 * @Description: //TODO
 * @Author: Michael Lynn
 * @CreateDate: 2020/7/13 20:59
 */
package cn.mlynn.androidfun.pagingsource;
...
public class SearchPagingSource extends ListenableFuturePagingSource<Integer, Article> {
    ...
    public SearchPagingSource(String k) {
        this.k = k;
    }
    ...
    public ListenableFuture<LoadResult<Integer, Article>> loadFuture(@NotNull LoadParams<Integer> loadParams) {
        ...
        ListenableFuture<LoadResult<Integer, Article>> pageFuture = Futures
                .transform(RetrofitManager.getInstance().createReq().getSearchResult(nextPageNumber, k)
                        , this::toLoadResult
                        , mBgExecutor);
